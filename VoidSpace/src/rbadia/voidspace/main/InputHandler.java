...
public class InputHandler implements KeyListener{
	...
	public void keyPressed(KeyEvent e) {
		...
		case KeyEvent.VK_SPACE:
			...
			if (status.getNewLevel()) {
				...
				if(status.getLevel() == 1)
					gameLogic.newLevel2Game();
				
				else if(status.getLevel() == 2)
					gameLogic.newLevel3Game();
				
				if(status.getLevel() == 3)
					gameLogic.newLevel4Game();
				
			    status.setNewLevel(false);
			}
			
		else if(!status.isGameStarted() && !status.isGameOver() && !status.isGameStarting()){
				// new game
				lastBulletTime = System.currentTimeMillis();
				leftIsPressed = false;
				rightIsPressed = false;
		...
		else if(status.numLevel() == 4 && status.isBossDestroyed()){
			// new game
			lastBulletTime = System.currentTimeMillis();
			leftIsPressed = false;
			rightIsPressed = false;
